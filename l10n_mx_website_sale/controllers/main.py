...
class WebsiteSale(WS):
    ...
    def address(self, **kw):
        ...
        # Only return city_id if a city is selected, otherwise js get the cities
        state_id = partner.state_id.id if partner_id != -1 else values['state_id'] if values.get('city_id') else 0
        if state_id:
            render_values['city_id'] = request.env['res.city'].sudo().search([('state_id', '=', int(state_id))])
